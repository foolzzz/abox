...
- [x] **浏览器 E2E 回归覆盖**：真实 Chrome 用稳定的可访问名称定位并执行 Box、Host、Agent Definition、Workspace 四类删除流程（包括权限、Host online/offline 和受保护删除冲突），并覆盖一步创建 Agent Box 与 PWA 新版本自动激活/重载；`npm --workspace apps/web run test:e2e` 共 7 项实际执行通过。
