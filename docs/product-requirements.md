...
- [x] Admin 选择 Claude Resume 时可从所选 Host 搜索本地 Session；选择结果自动填充 Session UUID，优先复用同 Host 上路径一致且 Ready 的已注册 Workspace；仅在 Workspace 未注册时切换到本地路径并在创建时验证，发现失败时保留手动输入。
- [x] 运行中的 Claude Background Session 支持多个 Box 重复 Attach；Web 提交规范 Session UUID，daemon 通过 `claude agents --json` 解析实际 Background Job ID 后执行 `claude attach`，Attachment 持久化记录 Box、Host、Session UUID、模式和共享输入策略；普通 Interactive Session 不作为可 Attach 目标。
- [x] Detach 只关闭当前 Box 的 tmux/Attachment，不停止共享 Claude；Admin“为所有人停止”通过 `claude stop` 停止 Session 并将全部关联 Box 转为 Resume。
...
- [x] **浏览器 E2E 回归覆盖**：真实 Chrome 覆盖 Box Owner 筛选、Claude Resume/Discovery/Shared Attach、Resume Workspace 自动复用、Attach Session UUID 提交、Box Detach、Admin 全局 Stop、Host 级联删除、Agent 默认模型、Workspace 自动注册、Terminal 无垂直边缝、Terminal 宽屏及 390px 手机/640px 动态视口/768px 平板适配、Runtime/PWA 和远程 HTTP `randomUUID` 兼容回退；隔离 Fixture Suite 共 25 项通过，本机部署 Suite 共 6 项通过。
