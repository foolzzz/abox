...
export function DashboardView() {
  ...
  const resource = useResource<DashboardData>(async (signal) => {
    const [agents, hosts, workspaces, boxes] = await Promise.all([
      api.listAgents(signal),
      api.listHosts(signal),
      api.listWorkspaces(signal),
      api.listBoxes(signal)
    ]);
    return { agents, hosts, workspaces, boxes };
  }, [canCreateBox]);
  ...
  return (
    <div className="page">
      ...
      <section className="metric-grid" aria-label="System summary">
        ...
        <MetricCard icon="workspace" label="Ready workspaces" value={String(readyWorkspaces)} detail={`${data.workspaces.length} registered workspaces`} tone="neutral" />
        <MetricCard icon="agent" label="Agent definitions" value={String(data.agents.length)} detail="Available runtime profiles" tone="neutral" />
      </section>
