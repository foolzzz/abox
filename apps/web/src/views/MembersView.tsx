...
export function MembersView() {
  ...
  const deleteAccount = async () => {
    ...
    setRequestError(undefined);
    try {
      await api.deleteAccount(member.id);
      resource.setData((current) => current ? { ...current, members: current.members.filter((item) => item.id !== member.id) } : current);
      setDeletingMember(undefined);
      notify(`${member.login} was deleted.`);
    } catch (error) {
  ...
  return (
    <div className="page">
            ...
            {data.members.map((member) => {
              ...
              return (
                <div className="data-table__row" role="row" key={member.id}>
                  <span role="cell" data-label="Account" className="member-cell"><span className="member-avatar member-avatar--small">{initials(member.displayName || member.login)}</span><span><strong>{member.displayName || member.login}{isCurrent ? " (you)" : ""}</strong><small>{member.login} · {member.hasPassword ? "password account" : "legacy identity · no local password"} · joined {formatDate(member.createdAt)}</small></span></span>
                  ...
                  <span role="cell" data-label="Actions">{!isCurrent ? <div className="table-actions">{member.hasPassword ? <Button variant="ghost" disabled={busy} onClick={() => setResetMember(member)}>Reset password</Button> : null}<Button variant="danger" icon="trash" disabled={busy} onClick={() => { setDeleteError(undefined); setDeletingMember(member); }}>Delete</Button></div> : <span className="permission-caption">{busy ? "Updating…" : "Current session"}</span>}</span>
                </div>
              );
            })}
          </div>
      ...
      <Modal
        ...
        title={`Delete ${deletingMember?.displayName || deletingMember?.login || "account"}?`}
        description="Disable this account, revoke its sessions, and suspend its organization membership. Audit history is retained."
        onClose={() => { if (!deleteBusy) { setDeleteError(undefined); setDeletingMember(undefined); } }}
        size="small"
      >
        <div className="confirm-dialog">
          <InlineAlert tone="warning">You cannot delete your own account or the last active administrator.</InlineAlert>
          {deleteError ? <InlineAlert>{deleteError}</InlineAlert> : null}
          <div className="modal__actions">
            <Button disabled={deleteBusy} onClick={() => { setDeleteError(undefined); setDeletingMember(undefined); }}>Cancel</Button>
            <Button variant="danger" icon="trash" busy={deleteBusy} onClick={() => void deleteAccount()}>Delete account</Button>
          </div>
        </div>
      </Modal>
    </div>
